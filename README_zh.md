# Claude for Chrome (研究预览版) / 邀请码 - v1.0.34 (Powered by Opus 4.5)
...
💻 如何配置 API Base URL 和 API Key？
右键单击 Claude 扩展程序的图标，然后点击 Options（选项）菜单，进入选项页面。

在选项页面上，点击 API configuration (internal)（API 配置 - 内部），然后在 Anthropic API Key 下方的输入框中输入您的 API Key。

如果您需要配置第三方 API Key，则需要配置 API Base URL。请登录到 https://cfc.aroic.workers.dev/，并在 Base URL 下方的输入框中进行配置。在某些版本中，此配置也可以直接在扩展程序的选项页面上进行。

一切就绪！尽情在 Chrome 中使用 Claude 吧。


## 📚 免责声明

**本代码库仅供教育和研究目的使用。** 所有代码的著作权归属于 Anthropic Inc。

源代码是通过浏览器开发者工具从公开可访问的资源中获取的。

如有任何版权方面的顾虑，请联系我们进行移除。
